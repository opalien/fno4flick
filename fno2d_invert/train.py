...
def accuracy(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for a, u in dataloader:
            ...
            a_pred = model(u.unsqueeze(1))  # Assuming model outputs shape (batch_size, 1, Nt, Nr)
            error: torch.Tensor = torch.nn.functional.mse_loss(a, a_pred)
...
def train_one_epoch(model: torch.nn.Module, 
    ...
    for a, u in dataloader:
        ...
        #print(f"Input shape: {u.shape}, Target shape: {a.shape}")

        a_pred = model(u.unsqueeze(1))  # Assuming model outputs shape (batch_size, 1, Nt, Nr)
        
        loss: torch.Tensor = torch.nn.functional.mse_loss(a, a_pred)        
        loss.backward()
        optimizer.step()
