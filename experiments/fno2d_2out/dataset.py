...
class Dataset(torch.utils.data.Dataset[tuple[EDPParameters, Tensor]]):
    ...
    def add_element(self, d: dict[str, Tensor | float | int]):
        ...
        P = d["P"] if isinstance(d["P"], Tensor) else torch.tensor(d["P"])
        P = P.float()
        Nt_, Ns_src = P.shape

        r_src = np.linspace(0.0, r_max, Ns_src, dtype=np.float64)
        r_in = np.linspace(0.0, R, Nr + 1, dtype=np.float64)
        r_out = np.linspace(R, r_max, Nr + 1, dtype=np.float64)

        P_np = P.numpy().astype(np.float64)
        P_in_np = np.stack([np.interp(r_in, r_src, P_np[t]) for t in range(Nt)], axis=0)
        P_out_np = np.stack([np.interp(r_out, r_src, P_np[t]) for t in range(Nt)], axis=0)
...
#
    ...
    def plot_element(self, idx: int, normalised: bool = True):
        ...
        P_in  = P2c[0]  # (Nt, Ns), r∈[0,R] → [0,1]
        P_out = P2c[1]  # (Nt, Ns), r∈[R,r_max] → [0,1]
...
def collate_fn(batch: list[tuple[EDPParameters, Tensor]]):
    ...
    for b, (p, P2c) in enumerate(batch):
        P2c = P2c.to(device=device, dtype=dtype)  # (2, Nt, Ns)
        ...
        X_q = torch.stack([Cin, Din, Dout, T1in, T1out, rho], dim=0)  # (6, Nt, Ns)
